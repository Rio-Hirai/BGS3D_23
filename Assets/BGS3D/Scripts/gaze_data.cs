        ...
        namespace Eye
        {
            ...
            public class gaze_data : MonoBehaviour
            {
                ...
                void Update()
                {
                    ...
                    get_gaze_data(); // こちらに入れた方がフレームレートを確保できる？
                ...
                public void get_gaze_data()
                {
                    ...
                    ////データをリストに格納
                    /// タイムスタンプ，タスク番号，焦点座標x，焦点座標y，瞳孔径・右，瞳孔径・左，開き具合・右，開き具合・左
                    script.tasklogs3.Add(script.test_time + "," + (script.task_num) + "," + (CombineFocus.point.x) + "," + (CombineFocus.point.y) +  "," + (RightPupiltDiameter) +  "," + (LeftPupiltDiameter) + "," + (RightOpenness) + "," + (LeftOpenness));
                }
            }
        }
    }
